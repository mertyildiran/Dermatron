  ...
  action: function() {
    ...
    if (cursor.count()) {
        cursor.forEach(function (visit) {
            ...
            xmlData += "<event id='" + visit._id + "' start_date='" + visit.visitDate + " " + visit.visitTime + "' end_date='" + date + " " + time + "' text='" + patients.findOne(visit.patientId).name + " " + patients.findOne(visit.patientId).surname + "' details='" + visit.reasonToVisit + "' />";
        });
    }
